 /*!
  * @file  comparatorGetTemperature.ino
  * @brief 使用比较器获取温度
  * @n 实验现象：温度在串口显示，比较状态也在串口显示，测量温度随环境温度改变而改变,
...
void setup()
{
  ...
  */
  if((state = mcp9808.setAlertOutputMode(COMPARATOR_OUTPUT_MODE)) == 0){
    Serial.println("设置比较器模式成功！");
  }else{
  ...
  */
  if((state = mcp9808.setLockState(CRIT_WIN_LOCK)) == 0){
